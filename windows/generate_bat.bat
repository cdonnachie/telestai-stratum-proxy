...
if NOT "%CURRENT_DIRECTORY:~-32%" == "\telestai-stratum-proxy\windows\" (
    if NOT "%CURRENT_DIRECTORY:~-37%" == "\telestai-stratum-proxy-main\windows\" (
        echo Error: Please run this batch file as-is from its original location in the telestai-stratum-proxy folder
	pause
        exit /B
    )
)
...
echo echo ==========================================================>>"%FILE_LOCATION%"
echo "%CURRENT_DIRECTORY%python_files\python.exe" "%CURRENT_DIRECTORY%..\telestai-stratum-proxy.py" --address %PROXY_IP% --port %PROXY_PORT% --rpcip %RPC_IP% --rpcport %RPC_PORT% --rpcuser %RPC_USERNAME%  --rpcpass %RPC_PASSWORD% %TESTNET% %SHOW_JOBS% %VERBOSE%>>"%FILE_LOCATION%"
