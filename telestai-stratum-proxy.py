...
class TemplateState:
    ...
    @property
    def tag(self):
        return '\x1b[0;36mtelestai\x1b[0m'
        ...
        async with session.post(f'{node_url}', data=json.dumps(data)) as resp:
            try:
                ...
                if state.height == -1 or state.height != height_int:
                    ...
                    if state.height == - 1 or height_int > state.height:
                        if not state.seedHash:
                            seed_hash = bytes(32)
                            for _ in range(height_int//EPOCH_LENGTH):
                                k = sha3.keccak_256()
                                k.update(seed_hash)
                                seed_hash = k.digest()
                            if verbose:
                        ...
                        elif state.height % EPOCH_LENGTH == 0:
                        ...
                        if state.height % EPOCH_LENGTH - (state.height - height_int) < 0:
                ...
                if new_block or new_witness or state.timestamp + 60 < ts:
                    ...
                    # to the max.
                    arbitrary_data = b'/telestai-stratum-proxy/'
                    ...
                    vout_to_dev = b'\x76\xa9\x14' + base58.b58decode_check(dev_address)[1:] + b'\x88\xac'
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="telestai-stratum-proxy",
                                     description="Stratum proxy to solo mine to TELESTAI node.")
    ...
    logger = logging.getLogger('TLS-logger')
